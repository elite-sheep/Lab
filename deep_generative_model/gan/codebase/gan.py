...
def loss_nonsaturating_g(g, d, x_real, *, device):
    ...
    g_theta_z = g(z)
    d_phi_z = d(g_theta_z)
    g_loss = F.binary_cross_entropy_with_logits(d_phi_z, torch.ones_like(d_phi_z))
...
def loss_nonsaturating_d(g, d, x_real, *, device):
    ...
    d_phi_x = d(x_real)
    d_phi_z = d(g_theta_z)
    d_loss_1 = F.binary_cross_entropy_with_logits(d_phi_x, torch.ones_like(d_phi_x))
    d_loss_2 = F.binary_cross_entropy_with_logits(d_phi_z, torch.zeros_like(d_phi_z))
